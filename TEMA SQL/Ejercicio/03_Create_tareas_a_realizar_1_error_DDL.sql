...
CONSTRAINT fab_pais_mayusculas_ck CHECK (nombre = upper (pais))
...
articulo VARCHAR2(20), --si aun siendo NOT NULL le queremos poner nosotros un nombre lo pondriamos: articulo VARCHAR2(20) contraint art_artic_NN NOT NULL,
...
CONSTRAINT art_categoria_ck CHECK (categoria IN ('primero','segundo','tercero')) -- faltaba 
);
...
CONSTRAINT ven_fab_fk FOREIGN KEY(cod_fabricante) REFERENCES cod_fabricantes(cod_fabricantes),
...
LA SOLUCION SERIA QUE ESTA TABLA DEBERIA IR DESPUES DE LAS DE FABRICANTES,ARITUCLOS Y TIENDAS*/
 ...
 ALTER TABLE tiendas
 ADD CONSTRAINT nombre_upper_ck CHECK (nombre=INITCAP (nombre))
 REFERENCES tiendas;
   
   --ALTER TABLE tiendas
   --ADD CONSTRAINT tien_nombre_ck CHECK (nombre=INITCAP (nombre));

 ALTER TABLE pedidos MODIFY (ped_unidades_ck CHECK [6]);
    --ALTER TABLE pedidos MODIFY (unidades_pedidas NUMBRER (6));
 ALTER TABLE ventas MODIFY (ven_unidades_ck CHECK [6]);
    --ALTER TABLE ventas MODIFY (unidades_vendidas NUMBRER (6));

 ALTER TABLE tiendas
 ADD CONSTRAINT provincia CHECK (provincia != 'TOLEDO');
   --ALTER TABLE tiendas
   -- ADD CONSTRAINT tien_prov_ck CHECK (provincia != 'TOLEDO');
 ...
 ALTER TABLE pedidos DROP (pvp);
   -- tambien se` puede ALTER TABLE pedidos DROP COLUMN PVP / ALTER TABLE ventas DROP COLUMN PVP;
   ALTER TABLE ventas DROP (pvp);
   
  
DESCRIBE USER_TABLES; --PARA CONOCER LAS COLUMNAS QUE TIENE LA VISTA DEL DICCIONARIO
